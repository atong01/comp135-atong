    ...
    for r in randList:
        for i, v in enumerate(pd):
            if v > r:
                ...
                break
    return (tsetx, tsetr)
def calcCut(data, bestCutLoc, len):
    cut = 0 
    m = bestCutLoc
    if m[0] == 0: # classify all one way, move cut to infty
        ...
        cut = sum([data[m[0]][0], data[m[0]-1][0]]) / 2 #support vectors
    return cut
...
def printResults(L, err):
    for i,l in enumerate(L, start=1):
        use = "used" if l[1] == 1 else "unused"
        print "Learner ", i, " : on Attribute ", l[0][0], " Cutoff: ", l[0][1], use
