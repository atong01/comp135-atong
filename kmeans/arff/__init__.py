...
class Nominal(str):
    """
    Since this module uses repr(obj) to serialize strings, this overriding
    of __repr__ is needed to avoid the quotation marks in nominals.
    
    Usage:
        writer.pytypes[Nominal] = '{firstThing, secondThing, third}'
        writer.write([Nominal('firstThing')])
    """
    def __repr__(self):
        return self
...
def load(fname):
#added by alex tong march 2015
    if type(fname) == str:
        with open(fname, 'r') as fhand:
            for item in Reader(fhand):
                yield item
    if type(fname) == file:
        for item in Reader(fname):
            yield item
...
class Writer(_LineWriter):
    def __init__(self, fhand, relation='untitled', names=None):
        self.fhand = fhand
